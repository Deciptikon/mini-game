...
- Синица. Самый лёгкий и самый скрытный персонаж. Имеет особые свойства с старта: перемещается по любым ячейкам. Имеет мало здоровья, и теряет одну единицу HP если несколько ходов подряд летит над озером или пустыней. Беспрепятственно и безлимитно перемещается над ячейками степи, леса или гор.
  Любит леса и горы.
  Избегает озёра и пустыни.
...
- Змея. Имеет высокий шанс двинуться в выбранном направлении (то есть, если Змея на текущий ход пошла "вверх", то с большей вероятностью она пойдёт "вверх" и на следующий ход).

- Черепаха. Аналогично синице, может перемещаться по воде, но имеет малую скорость и в воде пропускает каждый второй ход.
...
- 🌪️ Скорость. Величина пропорциональная количеству ходов в еденицу времени.
...
0. Базовый уровень сложности.
   Обычная локация на которой довольно легко выживать. Локация небольшого размера: **8х8**.
1. На локации появляются лёгкие погодные эффекты (утро/день/вечер/ночь).
...
4. На локацию добавляются тяжелые погодные эффекты (туман, ветер, дождь, ураган).
